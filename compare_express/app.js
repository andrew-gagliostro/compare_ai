...
app.post("/api_withdata", async function (req, res) {
  /*make a script that will take prompt of type string and links
   of string array and feed the prompt to gpt4 and return the result
  */
  try {
    ...
    */
    ...
    let messages = [
      ...
      },
      {
        role: "system",
        content:
          "You will be provided with a text representation of an array that contains summaries different things, and your task is to return a comparison of the things in the context of the previous prompt content",
      },
    ];
    ...
    const plainResponse = response
      ...
      .replace(/\\'/g, "'"); // Replace \\' with single quotes (if needed)
    res.status(200).json({
      success: "post call succeed!",
      url: req.url,
      body: req.body,
